...
| `pytorch_optimize.py` | **prepare(optimize=True)** + **train_step()**: TF32, AMP, loader defaults, one-call step - throughput, no hyperparameter changes | `autotrainer run pytorch_optimize.py` |
...
| `tensorflow.py` | scope(): auto strategy + global batch scaling | `autotrainer run tensorflow.py` |
| `sklearn.py` | prepare() on nested estimators, auto n_jobs | `python sklearn.py` |
| `xgboost.py` | prepare() + boost_params() for both APIs | `python xgboost.py` |
