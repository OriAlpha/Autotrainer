...
def prepare(model, dataloader=None, optimizer=None):
    ...
    if mod.startswith("torch") or _is_torch_module(model):
        from .backends.torch_backend import prepare as _p
        return _p(model, dataloader, optimizer)
    ...
    if mod.startswith(("xgboost", "lightgbm")):
        from .backends.boosting_backend import prepare as _p
        return _p(model)

    if mod.startswith("sklearn") or _is_sklearn_estimator(model):
        from .backends.sklearn_backend import prepare as _p
        return _p(model)
...
def auto(model, dataloader, **kwargs):
...
def find_lr(model, dataloader, loss_fn, **kwargs) -> float:
...
def tune(model, train_loader, val_loader, **kwargs):
